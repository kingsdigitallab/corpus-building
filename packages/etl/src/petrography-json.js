...
export function resolveAna(refLookup, type, subtype) {
  if (!subtype || subtype === "unverified" || subtype === "unspecified") {
    ...
    const warnings = ana
      ? []
      : [`Missing reference lookup for type="${type}" subtype="${subtype}"`];
      ...
      ({ anas, warnings }, s) => {
        ...
        return found
          ...
          : {
              ...
              warnings: [
                ...warnings,
                `Missing reference lookup for type="${type}" subtype="${s}"`,
              ],
            };
      },
      { anas: [], warnings: [] },
...
export async function buildRecords({
  ...
  for (const row of stoneRows) {
    ...
    if (subtypeCol === "unverified") {
      ...
      if (!detailSubtype) {
        preWarnings = [
          `No detail subtype found for ${isic} (blank subtype in stones CSV)`,
        ];
      }
    ...
    results.push({
      ...
      description,
      provenance,
      warnings: [...preWarnings, ...anaWarnings],
    });
  }
...
export async function buildPetrographyJson(paths) {
  ...
  async function readXml(isic) {
    try {
      const xml = await fs.readFile(
        path.join(paths.xmlDir, `${isic}.xml`),
        "utf-8",
      );
  ...
  console.log(
    `[petrography-json] Wrote ${sortedRecords.length} records to ${paths.output}`,
  );

  if (withWarnings.length > 0) {
    console.warn(
      `[petrography-json] ${withWarnings.length} record(s) have warnings — check the "warnings" field in the output JSON`,
    );
  }
}
